...
class Dataloader:
    ...
    def get_season(self, date_str):
        ...
        if (month == 4 and day >= 20) or (month > 4 and month < 6) or (month == 6 and day <= 20):
            return 1  # Spring
        elif (month == 6 and day >= 21) or (month > 6 and month < 8) or (month == 8 and day <= 21):
            return 2  # "Summer"
        elif (month == 8 and day >= 22) or (month > 8 and month < 10):
    ...
    def generate_train_test_data_all_quads(self, data_quad_list, corrected_hru=False,
        ...
        all_imgs_30m_daily, all_imgs_30m_static, all_target_sm, all_quads, all_dates = [], [], [], [], []
        count = 0
        # start_date = datetime.date(2018, 8, 2)
        # end_date = datetime.date(2018, 8, 10)
        # dates_hru = [start_date + datetime.timedelta(days=x) for x in range((end_date - start_date).days + 1)]
        # dates_hru = [date.strftime('%Y%m%d.tif') for date in dates_hru]
        # print("dates_hru: ", dates_hru)
        dates_hru = ['20240404.tif', '20240405.tif', '20240406.tif']
    ...
    def generate_meta_data(self, start_in=0, end_in=500, folder=None):
        quadhashes = os.listdir(self.hru_sm_path)
        random_seed = 42
        ...
        with open(path, 'w') as file:
            for quad in quadhashes:
                file.write(quad + '\n')
        cmd = ['setfacl', '-Rm', 'g:sustain:rwx', self.meta_path]
        subprocess.call(cmd)
        return quadhashes

    def load_all_datasets_in_memory(self, start_in=0, end_in=500, folder=None):
        if not os.path.exists(self.meta_path + "/quads_" + str(folder) + ".txt"):
            print("Generating meta_data....")
            quadhashes = self.generate_meta_data(start_in, end_in, folder)
        else:
            quadhashes = self.read_folder_quads(folder)
        total_quad = len(quadhashes)
        print(
            "Found total quadhashes {}  start_index: {}, end_index: {}".format(total_quad, start_in, end_in), flush=True)

        all_input_daily, all_input_static, all_target, all_quads, dates = self.generate_train_test_data(
            quadhashes)

        combined_data = list(
            zip(all_input_daily, all_input_static, all_target, all_quads, dates))
...
class QuadhashDataset(Dataset):
    def __init__(self, data, training=True):
        self.all_input_daily, self.all_input_static, self.all_target, self.all_quads, self.dates = zip(*data)
        if training:
...
class QuadhashDatasetAllQuads(Dataset):
    def __init__(self, training=True, one_per_quad=False, start_in = 0, end_in=500, corrected_hru=True, folder=None):
        ...
        if folder is None:
            ...
            quadhashes = read_folder_quads(folder)[start_in:end_in]

        total_quad = len(quadhashes)
        print("Testing quadhashes for {}/{}, start- {} end- {} ".format(len(quadhashes), total_quad, start_in, end_in))

        self.all_input_daily, self.all_input_static, self.all_quads, self.dates = self.data_loader.generate_train_test_data_all_quads(
